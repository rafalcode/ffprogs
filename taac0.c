 ...
 * @return Error code (0 if successful)
 */
static int init_resampler(AVCodecContext *inpccx,
                          AVCodecContext *outccx,
                          SwrContext **resccx)
{
        int error;
         ...
         */
        error = swr_alloc_set_opts2(resccx,
                                             &outccx->ch_layout,
                                              outccx->sample_fmt,
                                              outccx->sample_rate,
                                             &inpccx->ch_layout,
...
static int init_fifo(AVAudioFifo **fifo, AVCodecContext *outccx)
{
    ...
    if (!(*fifo = av_audio_fifo_alloc(outccx->sample_fmt,
                                      outccx->ch_layout.nb_channels, 1))) {
 ...
 * @return Error code (0 if successful)
 */
static int write_output_file_header(AVFormatContext *outfcx)
{
    int error;
...
static int decode_audio_frame(AVFrame *frame,
                              AVFormatContext *inpfcx,
                              AVCodecContext *inpccx,
                                  ...
                                  int frame_size)
{
    ...
    if ((error = av_samples_alloc_array_and_samples(converted_input_samples, NULL,
                                  outccx->ch_layout.nb_channels,
                                  frame_size,
                                  outccx->sample_fmt, 0)) < 0) {
        fprintf(stderr,
                                         ...
                                         int *finished)
{
    ...
    if (decode_audio_frame(input_frame, inpfcx,
                           inpccx, &data_present, finished))
        goto cleanup;
    ...
    if (data_present) {
        ...
        if (init_converted_samples(&converted_input_samples, outccx,
...
int main(int argc, char **argv)
{
    ...
    if (open_input_file(argv[1], &inpfcx,
                        &inpccx))
        goto cleanup;
    /* Open the output file for writing. */
    if (open_output_file(argv[2], inpccx,
                         &outfcx, &outccx))
        goto cleanup;
    /* Initialize the resampler to be able to convert audio sample formats. */
    if (init_resampler(inpccx, outccx,
                       &resccx))
        goto cleanup;
    ...
    while (1) {
        ...
        while (av_audio_fifo_size(fifo) < output_frame_size) {
            ...
            if (read_decode_convert_and_store(fifo, inpfcx,
                                              inpccx,
                                              outccx,
                                              resccx, &finished))
                goto cleanup;
        ...
        while (av_audio_fifo_size(fifo) >= output_frame_size ||
            ...
            if (load_encode_and_write(fifo, outfcx,
                                      outccx))
                goto cleanup;
        ...
        if (finished) {
            ...
            do {
                if (encode_audio_frame(NULL, outfcx,
                                       outccx, &data_written))
                    goto cleanup;
            } while (data_written);
            break;
